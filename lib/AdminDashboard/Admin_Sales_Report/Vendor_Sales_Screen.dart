...
import 'package:poultry_a2z/AdminDashboard/Admin_Sales_Report/Compnents/Rest_Apis.dart';
...
class _Venders_SalesState extends State<Venders_Sales> {
  ...
  void get_Vendor_Sales_Report_Data() async {
    ...
    restApis.getVendorSalesReport( user_id, start_date1, end_date1,baseUrl, admin_auto_id).then((value) {
      if (value != null) {
        if (mounted) {
          setState(() {
                          ...
                          child: GestureDetector(
                            ...
                            onTap: ()=>{
                              Navigator.of(context).push(MaterialPageRoute(builder: (context) =>
                                  Select_Date_Range(type: "Sales",onSaveCallback: onSelectRangeListener,)),)
  ...
  List<DataRow> getRows(List<GetVendorSalesReport> saleModel) => saleModel.map((GetVendorSalesReport saleModel) {
    final cells = [saleModel.date, saleModel.ordersCount, saleModel.grossSales, saleModel.discounts, saleModel.returns,
      saleModel.netSale, saleModel.shipping, saleModel.tax, saleModel.totalSales];
  ...
  generateCsv() async {
    ...
    for(int index=0; index<VendorSalesList!.length ;index++){
      GetVendorSalesReport dataModel=VendorSalesList![index];
      List<String> data=[];

      print('3');

      data.add(dataModel.date);
      data.add(dataModel.ordersCount.toString());
      data.add(dataModel.grossSales.toString());
      data.add(dataModel.discounts.toString());
      data.add(dataModel.returns.toString());
      data.add(dataModel.netSale.toString());
      data.add(dataModel.shipping.toString());
    ...
    final file = await new File('${tempDir.path}/sales${DateTime.now()}.csv').create();
