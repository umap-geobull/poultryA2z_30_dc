...
import 'package:poultry_a2z/grobiz_start_pages/welcome/select_status_model.dart';
...
class CSCPickerState extends State<CSCPicker> {
  ...
  Future<List<String?>> getStates() async {
    ...
    var takeState = widget.flagState == CountryFlag.ENABLE ||
        widget.flagState == CountryFlag.SHOW_IN_DROP_DOWN_ONLY
        ? response
        .map((map) => CountryName.fromJson(map))
        .where(
  ...
  Widget countryDropdown() {
    return Container(
      ...
      Text(_selectedCountry!, style: TextStyle(color: Colors.black, fontSize: 14),):
      Text('Select Country', style: TextStyle(color: Colors.black, fontSize: 14),),
    );
