...
class EditRecommendedStyle extends StatefulWidget {

  OnEditCallback onEditCallback;
  String id;

  @override
  _EditRecommendedStyle createState() => _EditRecommendedStyle(id);

  EditRecommendedStyle(this.onEditCallback, this.id);
}

class _EditRecommendedStyle extends State<EditRecommendedStyle> {
  ...
  Color backgroundColor=Colors.white,appLabelColor=const Color(0xff443a49),appHeaderColor=Colors.black87,
      webLabelColor=const Color(0xff443a49),webHeaderColor=Colors.black87,
  ...
  _EditRecommendedStyle(this.home_component_id);
  ...
  Future getDetails() async {
    ...
    if (response.statusCode == 200) {
      ...
      if(status==1){
        Recommended_Model homeSubcategoryDetails=Recommended_Model.fromJson(json.decode(response.body));
  ...
  setStyleData(GetHomeComponentList homeComponent) {
    ...
    String showCategory=homeComponent.showInCategory;

    selectedMainCategory=showCategory.split('|');

    if(homeComponent.showOnHome=='true'){
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title:const Text('Edit Recommended component',style: TextStyle(color: appBarIconColor,fontFamily:'Lato',fontSize: 16),),
        actions: [
          isEditApiProcessing==true?
          const SizedBox(
            width: 80,
