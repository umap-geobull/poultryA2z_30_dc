...
import '../../Vendor_Module/Vendor_add_Product/Components/Model/Get_SizeList_Model.dart';
import 'package:http/http.dart' as http;
import 'package:getwidget/getwidget.dart';

typedef OnSaveCallback = void Function(List<String> colors);

class Size_Filter_Screen extends StatefulWidget {
  OnSaveCallback onSaveCallback;
  String selectedSize;

  Size_Filter_Screen(this.onSaveCallback, this.selectedSize);

  @override
  _Size_Filter_ScreenState createState() => _Size_Filter_ScreenState();
}

class _Size_Filter_ScreenState extends State<Size_Filter_Screen> {
  List<GetSizeList> getSize_List_ = [];
  List<GetSizeList> selected_Size_model_ = [];
                    ...
                    Container(
                      ...
                      child: Checkbox(
                         onChanged: (value) {
                          if (mounted) {
                            setState(() {
                              if (isAdded(getSize_List_[index].id) == true) {
                                selected_size_List.remove(getSize_List_[index].id);
                                print(selected_size_List.toString());
                                widget.onSaveCallback(selected_size_List);
                              }
                              else {
                                selected_size_List.add(getSize_List_[index].id);
                                print(selected_size_List.toString());
                                widget.onSaveCallback(selected_size_List);
                              }
                            });
                        ...
                        value: isAdded(getSize_List_[index].id),
                      ),
  ...
  getSizeList(String baseUrl) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (status == 1) {
        ...
        getSize_List_=getSizeListModel.getSizeList;
      }
      else {
        print('empty');
      }
    ...
    if (isAdded(id) == true) {
      ...
      if (selected_size_List.length < 10) {
        ...
        Fluttertoast.showToast(
          msg: "Maximum 5 sizes can be selected",
