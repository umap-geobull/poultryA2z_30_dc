...
typedef OnSaveCallback = void Function(String min_price,String max_price);

class Price_Range_Screen extends StatefulWidget {
  OnSaveCallback onSaveCallback;
  String min_price;
  String max_price;


  Price_Range_Screen(this.onSaveCallback, this.min_price, this.max_price);

  @override
  _Price_Range_ScreenState createState() => _Price_Range_ScreenState(min_price,max_price);
}

class _Price_Range_ScreenState extends State<Price_Range_Screen> {
...
//  final _controller_max_value = TextEditingController();
  ...
  _Price_Range_ScreenState(this.min_price, this.max_price);
          ...
          Column(
            children: [
              Container(
                  ...
                  child: const Text("Select Price Range",
 ...
 /* changeMinPrice(String min){
    min_price=min;
    if(double.parse(min_price) < _currentRangeValues.end) {
      _currentRangeValues=RangeValues(double.parse(min_price), double.parse(max_price));
    }
  ...
  changeMaxPrice(String max){
    max_price=max;
    if(double.parse(max_price) > _currentRangeValues.start) {
      _currentRangeValues=RangeValues(double.parse(min_price), double.parse(max_price));
    }
...
*/
  ...
  void initState() {
    ...
    // startController.text=minValue.toString();
    // endController.text=max_price.toString();
   /* if(min_price.isNotEmpty){
      _controller_min_value.text=min_price;
    }

    if(max_price.isNotEmpty){
      _controller_max_value.text=max_price;
    }*/

    if(min_price.isNotEmpty){
      _starValue=double.parse(min_price);
    }

    if(max_price.isNotEmpty){
      _endValue=double.parse(max_price);
    }
